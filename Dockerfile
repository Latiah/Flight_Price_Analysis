...
#
# Built on the official Airflow image so the scheduler, api-server, dag-processor,
# and any ad hoc `docker compose run` command all execute the exact same
# dependency set — the pinned Airflow version is the single source of truth.
...
# versions known to work with Airflow 3.3.1 instead of the newest release.
